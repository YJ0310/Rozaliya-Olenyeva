    ...
    async execute(interaction) {
        ...
        if (interaction.isCommand()) {
            ...
            if (permission_roles && permission_roles.roles.size > 0) {
                ...
                if (member_role.size === 0 && !interaction.memberPermissions.has(`ADMINISTRATOR`)) return interaction.reply({content: `You don't have this permission`, ephemeral: true});;
            }
            
            // check the channel permission
            if(!interaction.member.permissionsIn(interaction.channel).has([`VIEW_CHANNEL`,`SEND_MESSAGES`])) return interaction.reply({content: `You don't have the permission to use this command in this channel. Please use another channel`, ephemeral: true});
