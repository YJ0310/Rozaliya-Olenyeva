...
module.exports = {
    ...
    async execute(interaction, options) {
        ...
        if (!interaction.member.voice.channel) {
            interaction.reply({ content: `Please connect any voice channel in this server before using this command.`, ephemeral: true });
            return;
        }
        const temp_category = await interaction.guild.channels.cache.get(`957439720806309960`);
        ...
        if (options.getSubcommand() === `open`) {
            ...
            try {
                ...
                if(member_channel) return interaction.reply({ content: `You already open a temp channel`, ephemeral: true });
                ...
                interaction.reply({ content: `channel opened`, ephemeral: true });
            } catch {
                console.error;
        ...
        if (options.getSubcommand() === `close`) {
            ...
            // if no data
            if (!channel_data) return interaction.reply({ content: `You don't have the temp channel`, ephemeral: true });
        ...
        if (options.getSubcommand() === `give`) {
            return interaction.reply({ content: `this command is under maintainance`, ephemeral: true });
            // command close
            const giver = interaction.user;
            const taker = options.getMember(`user`).user;
            if (taker.bot) return interaction.reply({ content: `Can't transfer temp channel to bot`, ephemeral: true });
            await temp_voice_channel_list.findOne({ owner_id: taker.id }).then(async x => {
                if (x) { return interaction.reply({ content: `You can't give someone who already have a temp channel`, ephemeral: true }) };
                const channel_data = await temp_voice_channel_list.findOne({ owner_id: giver.id });
                if (!channel_data) return interaction.reply({ content: `You don't have a temp channel`, ephemeral: true });
                try {
                    ...
                    taker.dmChannel.send({ content: `Hello, ${giver.tag} want to transfer his/her temp channel ownership to you. Please click the accept button in 1 minutes to complete the transferation`, components: [button] }).then(async (message) => {
                        interaction.reply({ content: `Pending...`, ephemeral: true });
        ...
        if (options.getSubcommand() === `size`) {
            ...
            return interaction.reply({ content: `User limit for <#${channel.id}> has set to ${size}`, ephemeral: true });
        }
        if (options.getSubcommand() === `private`) {
            ...
            if (member_permission.length = 0) return interaction.reply({content: `You don't have this permission to run this command`, ephemeral: true});
            }
            ...
            if (private === true) {
                ...
                return interaction.reply({ content: `Enable private mod for <#${channel.id}>`, ephemeral: true });
            }
            if (private === false) {
                await channel.permissionOverwrites.edit(interaction.guild.roles.everyone, {
                    "CONNECT": true
                });
                return interaction.reply({ content: `Disable private mod for <#${channel.id}>`, ephemeral: true });
            }
        }
        if (options.getSubcommand() === `secret`) {
            ...
            if (secret === true) {
                ...
                return interaction.reply({ content: `Enable secret mod for <#${channel.id}>`, ephemeral: true });
            }
            if (secret === false) {
                await channel.permissionOverwrites.edit(member_role, {
                    "VIEW_CHANNEL": true
                });
                return interaction.reply({ content: `Disable secret mod for <#${channel.id}>`, ephemeral: true });
            }
        }
        if (options.getSubcommand() === `allow_user`) {
            ...
            return interaction.reply({ content: `${action} permission for ${amount} member(s)`, ephemeral: true });
        }
    }
}